...
| `reminders.snooze_minutes`         | Пауза после «Отложить», если до старта не осталось непройденных offset’ов; иначе — до следующего offset (по умолчанию `5`) |
...
1. Процесс раз в `poll_seconds` читает календарь через EWS на горизонт `lookahead_hours`. Если среди уже известных встреч скоро offset 0 **или** скоро истекает «Отложить», следующий опрос будится раньше — к этому моменту (для offset 0 — к **началу минуты** старта, `:00`).
2. Для встреч с подходящим `include_response_types` проверяет окна `offsets_minutes` (с допуском `grace_after_seconds`). Для offset 0 по встречам, загруженным в **прошлых** опросах, порог — начало локальной минуты старта (не секунды из EWS).
...
4. Кнопки на карточке:
   ...
   - **Отложить** (`skip_action_label`) — закрывает карточку и планирует повтор. Порядок выбора срока:
     1. если между сейчас и началом есть ещё **не сработавший** `offsets_minutes` — до **ближайшего** такого offset’а (например с T−10 при `[10,8,5,0]` → на T−8);
     2. иначе если до начала меньше `snooze_minutes` — на **начало** встречи;
     3. иначе через `snooze_minutes` от нажатия (в т.ч. после начала встречи).
     Offset’ы с временем срабатывания ≤ момента повтора помечаются покрытыми (без дубля с snooze). Карточки **той же встречи** в очереди снимаются; другие встречи в очереди не трогаются;
...
5. После начала встречи (когда непройденных offset’ов до старта уже нет) «Отложить» сдвигает на полные `snooze_minutes`. Можно откладывать снова — срок каждый раз пересчитывается по тем же правилам, пока встреча ещё в CalendarView либо пока не нажать «Прекратить».
