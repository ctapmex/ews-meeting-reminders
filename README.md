...
1. Процесс раз в `poll_seconds` читает календарь через EWS на горизонт `lookahead_hours`. Если среди уже известных встреч скоро offset 0, следующий опрос может быть раньше — к **началу минуты** старта (`:00` секунд).
